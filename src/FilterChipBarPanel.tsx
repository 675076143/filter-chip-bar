  ...
  return (
    <div style={{ background: C.colorBgElevated, borderRadius: 8, boxShadow: '0 3px 6px -4px rgba(0,0,0,0.12)', maxHeight: 420, overflow: 'hidden', display: 'flex' }}>
      ...
      >
        ...
        {dropdown.footer && <div style={{ padding: '6px 12px', fontSize: 12, color: C.colorTextSecondary, fontWeight: 500, borderTop: '1px solid ' + C.colorBorderSecondary, margin: '4px 12px 0' }}>{dropdown.footer}</div>}
        {dropdown.history.length > 0 && (
          <>
            <div style={{ height: 1, background: C.colorBorderSecondary, margin: '8px 12px 4px' }} />
            <div style={{ padding: '2px 12px 2px', fontSize: 11, color: C.colorTextQuaternary, fontWeight: 600 }}>搜索历史</div>
            {dropdown.history.map((h) => (
              <div key={h.key} onMouseDown={(e) => { e.preventDefault(); h.onSelect(); }} style={{ ...row, color: C.colorText, cursor: 'pointer', position: 'relative' }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = '#f5f5f5'; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>
                <span style={{ fontSize: 11, color: C.colorTextQuaternary, flexShrink: 0 }}>🕐</span>
                <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{h.text}</span>
                <span style={{ fontSize: 10, color: C.colorTextQuaternary, flexShrink: 0 }}>{h.count}条</span>
